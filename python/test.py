...
def fuzzy_math(num1, operator, num2):
  if type(num1) != int or type(num2) != int:
    raise Exception('We need todo fuzzy math on ints')
...
class TestFuzzyMath:
  def test_non_input_for_num1(self):
    """ error = None
    try:
        fuzzy_math('hi', '+', 2)
  ...
  def test_non_input_for_num2(self):
    with pytest.raises(Exception) as exc_info:
      fuzzy_math(1, '+', 'hi')
    assert 'fuzzy math on ints' in str(exc_info)
  ...
  def test_multiplication_with_medium_result(self):
    assert 'medium-sized' in fuzzy_math(2, '*', 9)
    
  def test_multiplication_with_large_result(self):
    assert 'really big' in fuzzy_math(2, '*', 12)

  def test_invalid_operator(self):
    with pytest.raises(Exception) as exc_info:
      fuzzy_math(2, 'f', 12)
    assert "know how to do math with" in str(exc_info)